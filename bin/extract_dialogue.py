...
def main():
    ...
    if args.nprocesses > 1:
        ...
        with h5py.File(args.store, 'w') as store:
            for video in videos:
                video_file = video['mp4']
                subtitles_file = video['srt']
                uncompress_video(store, video_file, subtitles_file, skip_video=args.no_video)
...
def extract_audio(store, video_name):
    ...
    sound_dataset = store.create_dataset('sound', data=sound, chunks=True, compression='gzip', shuffle=True)
...
def extract_subtitles(store, subtitles_name):
    ...
    for subtitle in subtitles:
        ...
        colors.append(color)
        texts.append(text)
    subtitles_group = store.create_group('subtitles')
    subtitles_color_index = np.array([color for color, i in sorted(font_colors.items(), key=lambda x: x[1])],
                                     np.uint8)
    subtitles_group.create_dataset('color_index', data=subtitles_color_index)
    subtitles_group.create_dataset('colors', data=np.array(colors, dtype=np.int8))
    subtitle_dataset = subtitles_group.create_dataset('texts', shape=(len(texts),),
                                                      dtype=h5py.special_dtype(vlen=str))
    subtitle_dataset[:] = texts
    subtitles_group.create_dataset('times', data=np.array(times, dtype=np.float32))
...
def extract_video(store, video_name, chunksize=2 ** 24):
    ...
    video_dataset = store.create_dataset('video',
...
def uncompress_video(store, video_name, subtitles_name, skip_video=False, chunksize=2 ** 24):
        print("Extracting video {}".format(video_name))
        movie_group = store.require_group(os.path.basename(video_name))

        extract_audio(movie_group, video_name)
        extract_subtitles(movie_group, subtitles_name)
        if not skip_video:
            extract_video(movie_group, video_name, chunksize)
